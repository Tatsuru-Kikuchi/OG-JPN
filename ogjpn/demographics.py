"""
-------------------------------------------------------------------------------
Functions for generating demographic objects necessary for the OG-JPN model
...
def get_un_fert_data(
    ...
    one year) and by age. The country_id=392 is for Japan. These data
    ...
    if download:
        ...
        pop_target = os.path.join(DATA_DIR, "un_jpn_pop.csv")
        fert_target = os.path.join(DATA_DIR, "un_jpn_fert.csv")
...
def get_un_mort_data(
    ...
    country_id=392 is for Japan. These data come from the United Nations
    ...
    if download:
        ...
        pop_target = os.path.join(DATA_DIR, "un_jpn_pop.csv")
        infmort_target = os.path.join(DATA_DIR, "un_jpn_infmort.csv")
        deaths_target = os.path.join(DATA_DIR, "un_jpn_deaths.csv")
...
def get_wb_infmort_rate(
    ...
    if download:
        ...
        # Hard code the infant mortality rate for Japan from the most
        # recent year (2020)
        wb_infmort_rate = 1.564
...
def get_fert(totpers, start_year=2021, end_year=None, graph=False):
    ...
    # Get UN fertility rates for Japan for ages 15-49
...
def get_pop_objs(E, S, T, curr_year, GraphDiag=False):
    """
    This function produces the demographics objects to be used in the OG-JPN
    model package.
    Args:
