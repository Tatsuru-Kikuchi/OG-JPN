"""
Example script for setting policy and running OG-JPN.
"""
...
def main():
    ...
    base_dir = os.path.join(CUR_DIR, "OG-JPN-Example", "OUTPUT_BASELINE")
    reform_dir = os.path.join(CUR_DIR, "OG-JPN-Example", "OUTPUT_REFORM")
    ...
    p.update_specifications(
        json.load(
            open(
                os.path.join(
                    CUR_DIR, "..", "ogjpn", "ogjpn_default_parameters.json"
                )
            )
        )
    )
    ...
    ans.to_csv(os.path.join(CUR_DIR, "ogjpn_example_output.csv"))
    client.close()
